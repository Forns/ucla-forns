function [Action, Reward, Prob, Conds] = tsIntentObs(theta, K, uCount, T, allFactors, intentEqn, covariateIndexEqn, pObs, pExp)
...
% Perform seeding of observations
for arm=1:K
    f(arm, arm) = pObs(arm, 1);
    s(arm, arm) = pObs(arm, 2);
end
