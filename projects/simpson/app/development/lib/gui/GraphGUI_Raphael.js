...
var GraphGUI_SVG = Class.create({
	createEdgeShape : function( edge_type, el ){
		...
		el.dom.attr({'stroke':DAGitty.style[edge_type+"path"].stroke});
	...
	anchorEdgeShape : function( el ){
		...
		var doit = function(){
			...
			//el.dom[1].transform('r'+a+',0,0 t'+el.x2+','+el.y2);
		};
	...
	createVertexShape : function( vertex_type, el ){
		...
		el.oval = oval;
		//el.dom.appendChild( document.createTextNode(el.id) );
		
		/*
		var e = document.createElement( 'v:oval' );
		e.setAttribute("style", "width: 40px; height: 30px; top: -15px; left: -20px;");
		el.dom.appendChild( e );
		
		var e = document.createElement( 'v:oval' );
		e.setAttribute("style", "width: 40px; height: 30px; top: -15px; left: -20px;");
		e.setAttribute("fillcolor", this.style[vertex_type].fill);
		el.dom.appendChild( e );
		el.dom.style.position='absolute';
		el.dom.style.cursor='move';
		el.dom.style.border='1px solid black';
		el.dom.style.backgroundColor = this.style[vertex_type].fill;*/
